...
async fn test_task() {
    ...
    info!(
        "✅ TEST_INSTANT_NOW_OK: Instant::now() succeeded: {:?}",
        now1
    );

    // Small delay to see if time advances
    info!(
        "TEST_TIMER_1MS_START: Testing Timer::after(1ms) - should complete almost immediately, if not [TEST_TIMER_1MS_OK] logged before test ends, this test FAILED"
    );
    ...
    info!(
        "✅ TEST_TIMER_1MS_OK: Timer::after(1ms) completed successfully in {:?}",
        elapsed
    );
    ...
    if now2 > now1 {
        info!(
            "✅ TEST_TIME_ADVANCING_OK: Time is advancing correctly: {:?} > {:?}",
            now2, now1
        );
    ...
    info!(
        "TEST_TIMER_100MS_START: Testing Timer::after(100ms) accuracy test, if not [TEST_TIMER_100MS_OK] logged before test ends, this test FAILED"
    );
    let start = Instant::now();
    ...
    let elapsed = end.duration_since(start);
    info!(
        "✅ TEST_TIMER_100MS_OK: Timer::after(100ms) completed in {:?}",
        elapsed
    );
    ...
    if elapsed >= Duration::from_millis(90) && elapsed <= Duration::from_millis(110) {
        ...
        warn!(
            "⚠️  TEST_TIMER_100MS_ACCURACY_WARNING: Timer accuracy may be off: {:?}",
            elapsed
        );
    }
    ...
    info!(
        "TEST_MULTIPLE_TIMERS_START: Testing 3 sequential Timer::after(50ms) calls, if not [TEST_MULTIPLE_TIMERS_OK] logged before test ends, this test FAILED"
    );
    for i in 1..=3 {
        let start = Instant::now();
    ...
    info!(
        "TEST_TIMER_SHORT_START: Testing short Timer::after(2ms) for consistency, if not [TEST_TIMER_SHORT_OK] logged before test ends, this test FAILED"
    );
    let start = Instant::now();
    ...
    let elapsed = end.duration_since(start);
    info!(
        "✅ TEST_TIMER_SHORT_OK: Short timer (2ms) completed in {:?}",
        elapsed
    );
    ...
    info!(
        "TEST_TIMER_200MS_START: Testing Timer::after(200ms) stability test, if not [TEST_TIMER_200MS_OK] logged before test ends, this test FAILED"
    );
    let start = Instant::now();
    ...
    let elapsed = end.duration_since(start);
    info!(
        "✅ TEST_TIMER_200MS_OK: Long timer (200ms) completed in {:?}",
        elapsed
    );
    ...
    if elapsed >= Duration::from_millis(180) && elapsed <= Duration::from_millis(220) {
        ...
        warn!(
            "⚠️  TEST_TIMER_200MS_ACCURACY_WARNING: Long timer accuracy may be off: {:?}",
            elapsed
        );
    }
    ...
    info!(
        "🚀 TIMER_TEST_COMPLETE: embassy-time Timer::await functionality is working on HT32F52352 with interrupt executor!"
    );
}
