//! Test critical section implementation for Embassy compatibility
//!
//! This example validates that our critical section implementation
//! follows Embassy standards and works correctly with nested sections.
...
async fn main(_spawner: embassy_executor::Spawner) {
    ...
    defmt::info!("Starting critical section tests");
    ...
    // Test 3: Nested critical section safety
    test_nested_critical_sections().await;

    defmt::info!("✅ All critical section tests passed successfully");
}

async fn test_basic_critical_section() {
...
async fn test_nested_critical_sections() {
    defmt::info!("Testing nested critical sections, if you don't see `ok` outputs, it's failed");
