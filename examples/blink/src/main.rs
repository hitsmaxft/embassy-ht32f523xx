...
async fn blink_task() {
    ...
    loop {
        ...
        info!(
            "⏰ BLINK_CYCLE_{}_TIMER1_OK: First 500ms timer completed",
            cycle_count
        );
        ...
        Timer::after(Duration::from_millis(500)).await;
        info!(
            "⏰ BLINK_CYCLE_{}_TIMER2_OK: Second 500ms timer completed",
            cycle_count
        );

        info!(
            "✅ BLINK_CYCLE_{}_COMPLETE: Blink cycle {} completed successfully",
        ...
        if cycle_count % 10 == 0 {
            info!(
                "🎯 BLINK_MILESTONE: Successfully completed {} blink cycles ({} seconds total)",
                cycle_count, cycle_count
            );
        }
    }
